...
local np=UDim2.new(mf.Position.X.Scale,mf.Position.X.Offset+dx,mf.Position.Y.Scale,mf.Position.Y.Offset+dy)
if not isMin then
cf.Visible=false
tc.Visible=false
...
minBtn.MouseEnter:Connect(function()tw(minBtn,{BackgroundColor3=theme.pd},.2)end)
minBtn.MouseLeave:Connect(function()tw(minBtn,{BackgroundColor3=theme.tr},.2)end)
...
tw(mf,{Size=UDim2.new(0,0,0,0)},.4,Enum.EasingStyle.Back,Enum.EasingDirection.In)
task.delay(.5,function()
if sg and sg.Parent then sg:Destroy() end
end)
end)
closeBtn.MouseEnter:Connect(function()tw(closeBtn,{BackgroundColor3=theme.pd},.2)end)
closeBtn.MouseLeave:Connect(function()tw(closeBtn,{BackgroundColor3=theme.pr},.2)end)
...
local tColors={Default=theme.pr,Success=theme.su,Warning=Color3.fromRGB(255,165,0),Error=Color3.fromRGB(255,0,0)}
...
tw(n,{Position=UDim2.new(0,0,n.Position.Y.Scale,n.Position.Y.Offset)},.4,Enum.EasingStyle.Back,Enum.EasingDirection.Out)
...
btn.MouseEnter:Connect(function()tw(btn,{BackgroundColor3=Color3.fromRGB(35,35,35)},.2)end)
btn.MouseLeave:Connect(function()tw(btn,{BackgroundColor3=theme.tr},.2)end)
return btn
end
...
if c.Flag then Tab.Flags[c.Flag]=inp.Text end
inp.Focused:Connect(function()tw(inp,{BackgroundColor3=Color3.fromRGB(30,30,30)},.2)end)
inp.FocusLost:Connect(function(ent)
tw(inp,{BackgroundColor3=theme.bg},.2)
if ent then
if c.Flag then Tab.Flags[c.Flag]=inp.Text end
pcall(c.Callback or function()end,inp.Text)
end
end)
inp:GetPropertyChangedSignal("Text"):Connect(function()
if c.Flag then Tab.Flags[c.Flag]=inp.Text end
end)
fr.MouseEnter:Connect(function()tw(fr,{BackgroundColor3=Color3.fromRGB(35,35,35)},.2)end)
fr.MouseLeave:Connect(function()tw(fr,{BackgroundColor3=theme.tr},.2)end)
return{
Set=function(s,t)inp.Text=t if c.Flag then Tab.Flags[c.Flag]=t end end,
...
if c.Flag then Tab.Flags[c.Flag]=val end
tBtn.MouseButton1Click:Connect(function()
val=not val
...
if c.Flag then Tab.Flags[c.Flag]=v end
end
}
end
...
if c.Flag then Tab.Flags[c.Flag]=v end
pcall(c.Callback or function()end,v)
end
...
if c.Flag then Tab.Flags[c.Flag]=opt end
pcall(c.Callback or function()end,opt)
end)
ob.MouseEnter:Connect(function()
if opt~=val then tw(ob,{BackgroundColor3=Color3.fromRGB(35,35,35)},.2)end
end)
ob.MouseLeave:Connect(function()
if opt~=val then tw(ob,{BackgroundColor3=theme.tr},.2)end
end)
end
btn.MouseButton1Click:Connect(function()
open=not open
...
if c.Flag then Tab.Flags[c.Flag]=def end
local function updLbl()
local s={}
for o,i in pairs(sel)do if i then table.insert(s,o)end end
lbl.Text=(c.Name or"Multi")..": "..(#s>0 and table.concat(s,", ")or"None")
if c.Flag then Tab.Flags[c.Flag]=s end
end
for _,opt in ipairs(opts)do
...
local s={}
for o,i in pairs(sel)do if i then table.insert(s,o)end end
pcall(c.Callback or function()end,s)
end)
of.MouseEnter:Connect(function()tw(of,{BackgroundColor3=Color3.fromRGB(35,35,35)},.2)end)
of.MouseLeave:Connect(function()tw(of,{BackgroundColor3=theme.tr},.2)end)
end
btn.MouseButton1Click:Connect(function()
open=not open
if open then
...
Set=function(s,v)
sel={}
for _,i in ipairs(v)do sel[i]=true end
updLbl()
end
}
end
